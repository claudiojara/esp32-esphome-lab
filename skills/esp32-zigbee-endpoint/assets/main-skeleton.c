/*
 * Zigbee End Device skeleton for ESP32-C6 / ESP32-H2 with esp-zigbee-sdk.
 *
 * Validated against ESP-IDF v5.3.2 + esp-zigbee-lib ~1.6.0.
 * Customize: LED_GPIO, MANUFACTURER_NAME, MODEL_IDENTIFIER, HA_ENDPOINT.
 *
 * Pattern: ZED on-off light. Replace esp_zb_on_off_light_ep_create with the
 * appropriate helper (esp_zb_temperature_sensor_ep_create, etc) for sensors.
 */
...
#define LED_GPIO                    GPIO_NUM_15      // XIAO ESP32-C6: GPIO15 active LOW
...
static void zb_task(void *arg) {
    ...
    // Inject manufacturer + model into Basic cluster (post-creation, pre-register)
    ...
    // Dev: scan all channels. Once joining is reliable, narrow to coordinator's channel:
    //   esp_zb_set_primary_network_channel_set(1u << 11);   // channel 11 only
...
void app_main(void) {
    ESP_LOGI(TAG, "Zigbee on/off light starting");
